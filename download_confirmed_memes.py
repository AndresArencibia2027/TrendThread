...
Pages used (by default):
- https://knowyourmeme.com/memes?kind=confirmed&sort=newest       (page 1)
- https://knowyourmeme.com/memes/page/2?kind=confirmed&sort=newest (page 2)
"""
...
from confirmed_memes_pages import fetch_entries


OUTPUT_DIR = Path("downloaded_confirmed_memes")
...
def download_image(img_url: str, title: str, index: int, out_dir: Path) -> Path | None:
    ...
    print(f"   ⬇️  Downloading image -> {fname}")
    try:
...
if __name__ == "__main__":
    # Adjust the page range here if you want more than pages 1–2
    main(1, 2)
